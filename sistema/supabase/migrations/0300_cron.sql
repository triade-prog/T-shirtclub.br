...
-- quando há mensagem na fila para enviar. O endereço do worker fica em app_settings e o
...
-- Chama o worker se houver mensagem vencida na fila. Devolve se chamou.
...
begin
  if coalesce(v_url, '') = ''
     ...
     or not exists (select 1 from outbox_messages where status = 'PENDENTE' and next_attempt_at <= app_now()) then
    return false;
  end if;
  ...
  execute 'select net.http_post(url := $1, body := ''{}''::jsonb, headers := $2, timeout_milliseconds := 30000)'
    using v_url || '/outbox', jsonb_build_object('content-type', 'application/json', 'x-worker-segredo', v_segredo);
  return true;
end $$;
