...
**Telas da F2, em fatias (26/09):** fatia 1 pronta, o início editorial com os blocos do painel e o cartão de produto, gerado no servidor (`apps/web/src/lib/catalogo.ts` lê a api-public com o segredo de repasse). Fatia 2 pronta: `/colecao/[slug]` (filtro Todas, Disponíveis e Últimas peças por link) e `/produto/[slug]` (galeria de arrastar com miniaturas, preço com a oferta do Club, detalhes e looks); o botão "Adicionar ao Club" envia `GET /sacola?adicionar=<slug>`, que a fatia 3 vai tratar. Próxima: sacola.
