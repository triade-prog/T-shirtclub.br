...
**Telas da F2, em fatias (26/09):** fatia 1 pronta, o início editorial com os blocos do painel e o cartão de produto, gerado no servidor (`apps/web/src/lib/catalogo.ts` lê a api-public com o segredo de repasse). Fatia 2 pronta: `/colecao/[slug]` (filtro Todas, Disponíveis e Últimas peças por link) e `/produto/[slug]` (galeria de arrastar com miniaturas, preço com a oferta do Club, detalhes e looks); o botão "Adicionar ao Club" envia `GET /sacola?adicionar=<slug>`, que a fatia 3 vai tratar. Conferida com a V4 (26/09): o que faltava foi incluído (voltar, favoritar no navegador, adicionar rápido no cartão, faixa "The edit" e nota editorial); busca e favoritos no cabeçalho esperam páginas próprias. Fatia 3 pronta: `/sacola` gerada no servidor, com a sacola num cookie só da loja (`__Host-sacola`, em `lib/sacola.ts`; o desenho previa localStorage, trocado para funcionar sem JavaScript e sem piscar), "Adicionar" por `GET /sacola?adicionar=` tratado no `proxy.ts`, "Remover" por ação do servidor, preço e desconto pela cotação `POST /v1/cart/quote` e o número de peças no cabeçalho. "Reservar minhas peças" leva a `/reserva`. Fatia 4 pronta: `/reserva` (Seus dados: nome, WhatsApp, entrega pretendida, cupom com "Aplicar", aviso de privacidade e Turnstile) e `/reserva/codigo` (mensagem pronta para o WhatsApp da loja, acompanhamento a cada 2 s, campo do código e "Confirmar e reservar"), seguindo as telas 5 e 6 de `docs/design-frontend.html` e o protótipo 02 e 03, já que a V4 não tem essas telas. O navegador chama a api-public pelo `/api` (`lib/api.ts`), para os cookies da tentativa e da sessão. Com a reserva criada, a sacola se esvazia e a tela vai para `/reserva/[numero]`. Novas variáveis: `NEXT_PUBLIC_TURNSTILE_SITE_KEY` também na loja e `WHATSAPP_LOJA`. Fatia 5 pronta: `/reserva/[numero]` (telas 7 e 9 do design e o começo da 8, mais o protótipo 04) com cronômetro pelo relógio do servidor, "Gerar código PIX" e "Copiar código PIX", QR code para o computador, consulta do pagamento a cada 3 s, tolerância sem PIX novo, pedido de cancelamento, pago (passos até a entrega) e expirada. A reserva é aberta pela sessão da cliente; o id vem da confirmação ou de `/v1/me/reservations`. Falta o cartão (Card Payment Brick, F6.4). Fatia 6 pronta: com o pagamento confirmado, a mesma página mostra a escolha da entrega (retirada, motoboy ou envio, com endereço), o frete calculado pela loja e pago por PIX em até 2 h, frete vencido, o andamento, o código de retirada e o rastreio (`Entrega.tsx`, protótipo 19). O link `/r#chave` abre a reserva com a sessão só dela; para confirmar ou trocar a entrega por ele, a cliente pede o código no WhatsApp (D13, `VerificarWhatsApp.tsx`). O PIX virou um componente (`BlocoPix.tsx`), usado pelos produtos e pelo frete. Fatia 7 pronta: `/consulta` ("Minhas reservas" do rodapé, protótipo 05) pede o WhatsApp e o Turnstile, confirma com o código pelo WhatsApp e lista as reservas do número com o próximo passo de cada uma; com a sessão do telefone, a lista aparece direto. O passo do código fica em `app/_verificacao/CodigoWhatsApp.tsx`, usado também pela entrega pelo link. Com isso, o caminho da cliente está completo na loja. Fatia 8 pronta: pagamento com cartão pelo Card Payment Brick do Mercado Pago (`BlocoCartao.tsx`), com o SDK carregado só quando a cliente escolhe cartão, crédito à vista, só o token indo para a API e a recusa traduzida (`textoRecusaCartao` no domínio). A cliente escolhe PIX ou cartão, e a primeira cobrança fixa a forma, que vale também para o frete. Variável nova: `NEXT_PUBLIC_MP_PUBLIC_KEY`; sem ela, a loja oferece só o PIX. **A validar com a chave pública de teste:** este ambiente não alcança `sdk.mercadopago.com`, então o Brick foi testado com uma versão falsa; conferir se ele aparece com o visual certo com a CSP atual (`style-src` só com nonce). Falta: as telas do painel.
